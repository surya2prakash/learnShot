...
export default function Instructor() {
     ...
     const instructorBackend = async() =>{
          setLoading(true);
            const getInstructorApi = await getInstructorData();
            const result = await fetchInstructorCourse();
            
            console.log(getInstructorApi);

            if(getInstructorApi.length){
                  setInstructorData(getInstructorApi);  
            }
    ...
    const totalAmount = instructorData?.reduce((acc,curr)=>
        acc + (curr?.totalAmountGenrated || 0),0 
    );

    const totalStudents = instructorData?.reduce((acc,curr)=> acc +( curr?.totalStudentEnrolled || 0),0);
                   ...
                   <div className='my-4 flex h-[450px] space-x-4'>
                        {totalAmount > 0 ||  totalStudents > 0 ? (<InstructorChart courses={instructorData}/>) : (
                         <div className="flex-1 rounded-md bg-richblack-800 p-6">
                              ...
                              <p className="mt-4 text-xl font-medium text-richblack-50">Not Enough Data To Visudalize</p>
                         </div>
                        )} 
